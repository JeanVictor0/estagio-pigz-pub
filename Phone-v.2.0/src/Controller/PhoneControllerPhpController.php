...
class PhoneControllerPhpController extends AbstractController
{
    ...
    public function index(ManagerRegistry $doctrine,Request $request, $id)
    {
        ...
        if (strlen($request['number']) < 10){
            return New Response('Numero invalido: Menos de 10 caracteres',$status = 400);
        } else if (strlen($request['number']) > 20){
            return New Response('Numero invalido: Numero com quantidade acima de caracteres',$status = 400);
        } else if(intval($request['number']) <= 1){
            return New Response('Numero invalido: Deverá ter apenas numeros e não outros caracteres',$status = 400);
        }
